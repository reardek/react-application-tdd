...
const ReviewsContextProvider = ({ children }) => {
  ...
  const getReviewsRequest = async id => {
    const result = await Api.fetchData(
      `https://my-json-server.typicode.com/reardek/react-application-tdd/hotels/${id}/reviews`,
    );
  ...
  const addReviewRequest = async content => {
    const result = await Api.postData(
      'https://my-json-server.typicode.com/reardek/react-application-tdd/hotels',
      content,
    );
